    ...
    >
      ...
      <div className="mb-12 flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
        ...
        <p className="max-w-sm text-sm leading-relaxed text-muted-foreground md:text-right">
          Three powerful tools, one slick app — everything a modern style enthusiast needs.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
        {features.map((feature) => {
          ...
          return (
            ...
            >
              {/* Icon */}
              <div className={`flex h-12 w-12 items-center justify-center rounded-2xl bg-white/20 ${feature.iconColor}`}>
                <Icon className="h-6 w-6" aria-hidden="true" />
              </div>

              {/* Tag */}
              <span className={`w-fit rounded-full px-3 py-1 text-xs font-bold uppercase tracking-wider ${feature.tagColor}`}>
                {feature.tag}
              </span>

              {/* Content */}
              <div className="flex flex-col gap-2 mt-auto">
                <h3 className="font-serif text-2xl font-bold text-foreground">{feature.title}</h3>
