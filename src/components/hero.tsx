        ...
        <div className="grid grid-cols-1 items-end gap-10 lg:grid-cols-2">
          ...
          <div className="flex flex-col gap-7 pb-14 md:pb-20 animate-fade-in-up">
            ...
            <p className="max-w-md text-base leading-relaxed text-muted-foreground md:text-lg">
              Build fire outfits, discover streetwear drops, and flex your look to a community of
              fashion-forward creators. BejoLooks is the app your closet has been waiting for.
            </p>
            ...
            <div className="flex flex-wrap items-center gap-3">
              <Link
                href="/register"
                className="inline-flex items-center gap-2 rounded-full bg-accent px-7 py-3.5 text-sm font-bold text-accent-foreground shadow-sm transition-all hover:scale-105 active:scale-95"
              >
                Start for Free
              ...
              <Link
                href="/explore"
                className="inline-flex items-center gap-2 rounded-full border-2 border-border bg-transparent px-7 py-3.5 text-sm font-bold text-foreground transition-colors hover:border-foreground"
              >
                Browse Looks
              </Link>
            </div>
            ...
            <div className="flex items-center gap-8 border-t border-border pt-7">
              {stats.map((s, i) => (
                <div key={s.label} className={`flex flex-col gap-0.5 ${i !== 0 ? 'border-l border-border pl-8' : ''}`}>
                  <span className="font-serif text-2xl font-bold text-foreground">{s.value}</span>
          ...
          <div className="relative animate-fade-in-up animation-delay-200">
            ...
            <div className="absolute left-6 top-6 z-10 flex items-center gap-2 rounded-full bg-accent/90 backdrop-blur px-4 py-2 shadow-lg">
              <span className="h-2 w-2 rounded-full bg-primary animate-pulse" aria-hidden="true" />
              <span className="text-xs font-bold text-accent-foreground">Live: 230 styling now</span>
            </div>
            ...
            <div className="absolute right-6 top-1/3 z-10 rounded-2xl bg-white/90 backdrop-blur-sm border border-border px-4 py-3 shadow-xl">
              <p className="text-xs font-bold uppercase tracking-widest text-primary mb-0.5">Top Pick</p>
              <p className="text-sm font-bold text-foreground">Urban Cargo Look</p>
              <p className="text-xs text-muted-foreground">+2.4K saves this week</p>
            </div>
